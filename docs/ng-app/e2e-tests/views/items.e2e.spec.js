...
describe('products page', function () {
    ...
    function getProductsCount() {
        return element.all(by.repeater('item in vm.itemStorage.items'))
            .then(function (products) {
                return products.length;
            })
    }

    it('should display proper products amount', function () {

        // the default situation
        var itemsPerPage = element(by.model('vm.itemsControl.itemsPerPage'));
    ...
    it('should add product', function () {
        ...
        var btnAddProduct = element(by.css('a[ng-click="vm.itemStorage.add()"]'));
        btnAddProduct.click();
        ...
        // product searching
        var search = element(by.model('vm.itemsControl.search'));
    ...
    it('should pagination work properly', function () {
        var expectedBricks = element(by.binding('vm.itemStorage.items.length')).getText()
            .then(function (value) {
                var allItemsValue = value.match(/[0-9]/g).join('');
                var itemsPerPageSelect = element(by.model('vm.itemsControl.itemsPerPage'));
